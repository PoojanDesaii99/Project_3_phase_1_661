...
def C2G(node):
    distance = math.sqrt((node[0] - x2)**2 + (node[1] - y2)**2)
    return distance
...
for x in range(canvas.shape[1]): 
    for y in range(canvas.shape[0]):
        ...
        # Hexagon
        if (x>=(300-75*math.cos(np.deg2rad(30))) and x<=(300+75*math.cos(np.deg2rad(30))) and 
            (y-0.58*x-26.79)<=0 and (y+0.58*x-373.21)<=0 and (y-0.58*x+123.21)>=0 and (y+0.58*x-223.21)>=0):
            canvas[y,x] = red
            
        # Top Rectangle
        if x>=100 and x<=150 and y>=150 and y<=height: 
            canvas[y,x] = red
...
for x in range(canvas.shape[1]):
    for y in range(canvas.shape[0]):
        if (all(canvas[y,x] == red)) or (x<=-9 or x>=609 or y<=-9 or y>=259): 
...
while opennode_list != {}:
    ...
    else:    # if the goal node is not reached applying action:
        ...
        for n in next_nodes:
            if (n[0] == True) and (n[1] not in closednode_list):
                ...
                if n[1] not in opennode_list:
                    opennode_list[n[1]] = [c2c+M, wt*C2G(n[1]),active_node]                           # Adding the node to opennode list
                else:
                    if opennode_list[n[1]][0] + opennode_list[n[1]][1] > c2c+ M + wt*C2G(n[1]):       # Calculating the total cost
                        opennode_list[n[1]] = [c2c+M, wt*C2G(n[1]), active_node]
                        
    # Print start and goal nodes on canvas
    cv2.circle(canvas, (start_node[0],start_node[1]), 5, purple, -1)
...
print("The Total Runtime is:  ", total_time) 
